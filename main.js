...
try {
  ...
  // Load Key Model (add this with your other model loading)
let keyModel = null;
loadModelOptimized(
  'key.glb',
  function (gltf) {
    ...
    keyModel = gltf.scene;
    keyModel.scale.set(0.1, 0.1, 0.1);
    
    keyModel.position.set(-10, 1, 5);
    keyModel.rotation.set(0, Math.PI/4, 0);
    
    // Register as collectible item
    if (inventorySystem) {
      inventorySystem.registerCollectibleItem(keyModel, {
        name: 'Golden Key',
    ...
    scene.add(keyModel);
    registerModelWithWorldBuilder('key', gltf);
...
loadModelOptimized(
  ...
  function (gltf) {
    ...
    doorModel.scale.set(0.4, 0.4, 0.4);
    
    doorModel.position.set(15, 0, -10);
    doorModel.rotation.set(0, 0, 0);
    
    // Register as interactive object
    if (inventorySystem) {
      inventorySystem.registerInteractiveObject(doorModel, 'Golden Key', function(door, item) {
    ...
    scene.add(doorModel);
    registerModelWithWorldBuilder('door', gltf);
  ...
  function checkPortalView() {
    ...
    if (currentScene === 'main') {
      ...
      if (keyModel && inventorySystem) {
  keyModel.traverse((child) => {
    if (child.isMesh) {
      child.userData.parentKey = keyModel;
...
// Check for door
if (doorModel && inventorySystem) {
  doorModel.traverse((child) => {
    if (child.isMesh) {
      child.userData.parentDoor = doorModel;
    ...
    const keyIntersects = raycaster.intersectObjects(keyMeshes);
let targetKey = null;
...
if (targetKey && targetKey.userData.collectible) {
  ...
  const keyInfoWindow = document.createElement('div');
  keyInfoWindow.style.cssText = `
    position: fixed;
    top: 20px;
    left: 20px;
    background: rgba(139, 69, 19, 0.95);
    ...
    border-radius: 10px;
  `;
  keyInfoWindow.innerHTML = `
    <div style="color: #d4af37; font-weight: bold;">🗝️ ${targetKey.userData.itemData.name}</div>
    <div style="color: #90ee90; font-size: 12px; margin-top: 5px;">Press E to collect</div>
  `;
  
  // Remove existing key info windows
  document.querySelectorAll('#key-info-temp').forEach(el => el.remove());
  keyInfoWindow.id = 'key-info-temp';
  document.body.appendChild(keyInfoWindow);
...
if (targetDoor && inventorySystem) {
  ...
  if (hint) {
    const doorInfoWindow = document.createElement('div');
    doorInfoWindow.style.cssText = `
      position: fixed;
      top: 20px;
      left: 20px;
      background: rgba(64, 64, 64, 0.95);
    ...
    doorInfoWindow.id = 'door-info-temp';
    document.body.appendChild(doorInfoWindow);
...
// Clean up info windows if no interaction targets
document.querySelectorAll('#key-info-temp, #door-info-temp').forEach(el => el.remove());
